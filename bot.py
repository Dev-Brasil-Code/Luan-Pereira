...
class Chat:
    def __init__(self, msg):
        ...
        self.user_input = self.user_input.replace('@DevBrasilCodeOfc_bot', '')
        self.user_name = msg['from']['first_name']
        self.message_id = msg['message_id']

        self.messages = {
            'start':'🤖 Ola, '+ self.user_name +'!\n\n'
                    '📩 Envie me:\n\n'
                    '"*/music* _Nome da musica_"  or\n'
                    '"*/music* _Cantor- Nome música_"\n\n'
                    'Para pedir alguma música. 🎶',
            
            'spotify_input_error':"‼️ *Oops!O Bot não suporta links do Spotify!*\n"
                    'Try: "*/music* _Nome da musica_"\n'
                    'or: "*/music* _Cantor - Nome da musica_"',

            'invalid_command':'‼️ *Oops! Comando invalido!*\n'
                    'Try: "*/music* _Nome da música_"\n'
                    'or: "*/music* _Cantor - Nome da música_"',

            'too_long':'‼️ *Oops! Vídeo muito longo para converter!*\n'
                    'Peça algo de 30 minutos ou menos.'
    ...
    def process_request(self, user_input):
        ...
        if int(min_duration) < 30 and split_count < 3:
            file_name = Music.get_title(self, result) +' - @DevBrasilCodeOfc_bot '+str(randint(0,999999))+'.mp3'
            ...
            try:
                ...
                self.send_message('✅ Sucesso musica!')
                print ("\nSucesso!\n")
            except:
                print("\nError")
